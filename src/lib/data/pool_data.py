...
piada_history = pd.DataFrame(kp.get_pool_history(piada)).fillna(0)
...
pool_data = dict(pledge=pledge, total_delegated=total_delegated, number_of_delegators=number_of_delegators, 
                pool_fee=pool_fee, block_count=block_count, epoch=epoch, delegate_rewards=delegate_rewards, 
                ten_epoch_ros=ten_epoch_ros, life_time_ros=life_time_ros)
